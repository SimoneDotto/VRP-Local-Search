...
namespace EasyLocal
{
  ...
  namespace Debug
  {
    ...
    /** A Move Tester allows to test the behavior of a given
     ...
     @ingroup Testers
     */
    template <class Input, class Output, class State, class Move, class CostStructure = Core::DefaultCostStructure<double>>
    class MoveTester
    ...
    void MoveTester<Input, Output, State, Move, CostStructure>::CheckNeighborhoodCosts(const State &st) const
    {
      ...
      do
      {
        ...
        for (size_t i = 0; i < sm.CostComponents(); i++)
        {
          if (!IsZero(static_cast<int>(error.all_components[i])) && std::abs(error.all_components[i]) > tolerance)
          {
            error_found = true;
